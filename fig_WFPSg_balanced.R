...
g.1 <- ggplot(toplot, aes(days.adjusted, WFPSg))
g.1 <- (g.1
...
################
####
# WFPSg kernel density estimate by rain event
...
toplot[,myWFPSg:= WFPSg]
toplot[is.na(days.adjusted),myWFPSg:=NA]
toplot[days.adjusted < 0, myWFPSg:=NA]

toplot <- toplot[!is.na(event)]
to.plot.tt <- toplot[tillage=="T"]
to.plot.nt <- toplot[tillage=="NT"]

# d <- density(toplot[event=="0 - 15" & tillage=="T", myWFPSg], adjust = 1/2)
# d <- density(toplot[event=="0 - 15", myWFPSg], adjust = 1/2)
d <- density(toplot$myWFPSg, adjust = 1/2)# bw.nrd0(toplot$myWFPSg) to know the default bw for our data
myd <- data.table(myWFPSg=d$x, y=d$y)
# myd[,y:=6*y]
...
g.2 <- ggplot(toplot, aes(x= myWFPSg))
g.2 <- (g.2
...
myplot <- paste0(folder_out,"/compilation_WFPSg_balanced_3h_interp.png")
