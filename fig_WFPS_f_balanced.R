...
g.1 <- ggplot(toplot, aes(days.adjusted, WFPS_f))
g.1 <- (g.1
...
################
####
# WFPS_f kernel density estimate by rain event
...
toplot[,myWFPS_f:= WFPS_f]
toplot[is.na(days.adjusted),myWFPS_f:=NA]
toplot[days.adjusted < 0, myWFPS_f:=NA]

toplot <- toplot[!is.na(event)]
to.plot.tt <- toplot[tillage=="T"]
to.plot.nt <- toplot[tillage=="NT"]

# d <- density(toplot[event=="0 - 15" & tillage=="T", myWFPS_f], adjust = 1/2)
# d <- density(toplot[event=="0 - 15", myWFPS_f], adjust = 1/2)
d <- density(toplot$myWFPS_f, adjust = 1/2)# bw.nrd0(toplot$myWFPS_f) to know the default bw for our data
myd <- data.table(myWFPS_f=d$x, y=d$y)
# myd[,y:=6*y]

# plot(toplot$myWFPS_f)
# 
...
g.2 <- ggplot(toplot, aes(x= myWFPS_f))
g.2 <- (g.2
...
myplot <- paste0(folder_out,"/compilation_WFPS_f_balanced_3h_interp.png")
