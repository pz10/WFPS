---
title: "N2O ( 1st : total ) priming differences . Approaches: Non-parametric and parametric (no transformation)"
...
par(mfrow=c(1,4))
with(data, boxplot(N2O.1.123 ~ fertilization, main = "fertilization"))
with(data, boxplot(N2O.1.123 ~ precipitation, main = "precipitation"))
with(data, boxplot(N2O.1.123 ~ tillage, main = "tillage"))
with(data, boxplot(N2O.1.123 ~ incubation, main = "incubation"))


par(mfrow=c(1,4))
ggplot(data=data, aes(x=fertilization, y=N2O.1.123)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=precipitation, y=N2O.1.123)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=tillage, y=N2O.1.123)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
ggplot(data=data, aes(x=incubation, y=N2O.1.123)) + 
        geom_point(size=3, color="darkgrey", alpha=0.5) +
        theme_bw()
...
```{r}
fit <- lm(N2O.1.123 ~ fertilization + precipitation + tillage, data=data)
fit.aov <- aov(N2O.1.123 ~ fertilization + precipitation + tillage, data=data)
fit.aov.int <- aov(N2O.1.123 ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(N2O.1.123 ~ precipitation + tillage, data=data)
fit2 <- lm(N2O.1.123 ~ fertilization + tillage, data=data)
fit3 <- lm(N2O.1.123 ~ fertilization + precipitation, data=data)

fit.int <- lm(N2O.1.123 ~ (fertilization + precipitation + tillage)^2, data=data)
...
```{r}
fit.p <- lmp(N2O.1.123 ~ fertilization + precipitation + tillage, data=data)
fit.aovp <- aovp(N2O.1.123 ~ fertilization + precipitation + tillage, data=data)
fit.aovp.int <- aovp(N2O.1.123 ~ (fertilization + precipitation + tillage)^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(N2O.1.123 ~ precipitation + tillage, data=data)
fit2.p <- lmp(N2O.1.123 ~ fertilization + tillage, data=data)
fit3.p <- lmp(N2O.1.123 ~ fertilization + precipitation, data=data)

fit.int.p <- lmp(N2O.1.123 ~ (fertilization + precipitation + tillage)^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(N2O.1.123 ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(N2O.1.123 ~ fertilization + precipitation + tillage, data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(N2O.1.123 ~ fertilization + precipitation + tillage, data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(N2O.1.123 ~ fertilization + precipitation + tillage, data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(N2O.1.123 ~ fertilization + precipitation + tillage, data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
# myoutliers <- c(68,54)
# data[myoutliers, .(id, N2O.1.123, N2O.1.3, N2O.1.123, N2O.1.3)]
# 
...
summary(powerTransform(data[,N2O.1.123]))
```
...
### linearity
################################################################################
# boxTidwell(N2O.1.123 ~ fertilization + precipitation + tillage, data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(N2O.1.123 ~ (precipitation + tillage + fertilization), data=data, nbest=4)
...
```{r, echo=F}
# precipitation
results.rain <- W.MW.oneway(N2O.1.123 ~ precipitation, data, n=my.n)
summary(results.rain)
plot(results.rain)

# tillage
results.tillage <- W.MW.oneway(N2O.1.123 ~ tillage, data, n=my.n)
summary(results.tillage)
plot(results.tillage)

# fert
results.fert <- W.MW.oneway(N2O.1.123 ~ fertilization, data, n=my.n)
...
```{r, echo=F}
# precipitation
results.rain <- PToneway(N2O.1.123 ~ precipitation, data, n=my.n)
summary.oneway(results.rain)

# tillage
results.till <- PToneway(N2O.1.123 ~ tillage, data, n=my.n)
summary.oneway(results.till)

# fert
results.fert <- PToneway(N2O.1.123 ~ fertilization, data, n=my.n)
