---
title: "N2O ( 1st : total ) priming differences . Approaches: Non-parametric and parametric (no transformation; only precipitation)"
...
```{r}
fit <- lm(N2O.1.123 ~ precipitation , data=data)
fit.aov <- aov(N2O.1.123 ~ precipitation , data=data)
fit.aov.int <- aov(N2O.1.123 ~ (precipitation )^2, data=data)

# factor removal; for later model comparison
fit1 <- lm(N2O.1.123 ~ precipitation , data=data)
fit2 <- lm(N2O.1.123 ~ fertilization , data=data)
fit3 <- lm(N2O.1.123 ~ precipitation, data=data)

fit.int <- lm(N2O.1.123 ~ (precipitation )^2, data=data)
...
```{r}
fit.p <- lmp(N2O.1.123 ~ precipitation , data=data)
fit.aovp <- aovp(N2O.1.123 ~ precipitation , data=data)
fit.aovp.int <- aovp(N2O.1.123 ~ (precipitation )^2, data=data)

# factor removal; for later model comparison
fit1.p <- lmp(N2O.1.123 ~ precipitation , data=data)
fit2.p <- lmp(N2O.1.123 ~ fertilization , data=data)
fit3.p <- lmp(N2O.1.123 ~ precipitation, data=data)

fit.int.p <- lmp(N2O.1.123 ~ (precipitation )^2, data=data)
...
# 
# # outlier removed
# myfit <- lm(N2O.1.123 ~ precipitation , data=data[-c(6, 48,66)])
# gvmodel <- gvlma(myfit)
# summary(gvmodel)
# ```
...
# next.outlier <- lmp(N2O.1.123 ~ precipitation , data=data[-c(66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(N2O.1.123 ~ precipitation , data=data[-c(48,66)]) 
# outlierTest(next.outlier)
# 
# next.outlier <- lmp(N2O.1.123 ~ precipitation , data=data[-c(6, 48,66)]) 
# outlierTest(next.outlier)
# next.outlier <- lmp(N2O.1.123 ~ precipitation , data=data[-c(68,54)]) # only 17 is an outlier
# outlierTest(next.outlier)
# 
# 
# # identify outliers
...
### linearity
################################################################################
# boxTidwell(N2O.1.123 ~ precipitation , data=data)
...
```{r}
library(leaps)
leaps <-regsubsets(N2O.1.123 ~ (precipitation  + fertilization), data=data, nbest=4)
