...
g.2 <- (g.2
        + theme_bw(base_size = 8)
        + theme(plot.margin= unit(c(0.25, 1, 0, 0), "lines"),
                axis.ticks = element_line(size = 0.1),
                legend.position =   "none",
                panel.margin =       unit(-0.4, "lines"),
                strip.background =   element_blank(),
        ...
        # + ylab("occurrence by rain event [%]")
        + ylab("occurrence by incubation stretch [%] ")
...
#
myplot <- paste0(myplots,"/_WFPS_vs_NO_N2O_wfpsbyevent_w", window,".png")
...
print(g.1, vp = vplayout(1, 5:12))
print(g.1a, vp = vplayout(2, 5:12))
print(g.2, vp = vplayout(1:2, 1:4))
